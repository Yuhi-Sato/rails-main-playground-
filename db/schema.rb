...
ActiveRecord::Schema[8.2].define(version: 2026_04_18_014125) do
  ...
  create_trigger :logidze_on_posts, sql_definition: <<-SQL
      CREATE TRIGGER logidze_on_posts BEFORE INSERT OR UPDATE ON public.posts FOR EACH ROW WHEN ((COALESCE(current_setting('logidze.disabled'::text, true), ''::text) <> 'on'::text)) EXECUTE FUNCTION logidze_logger('50', 'updated_at')
  SQL
end
